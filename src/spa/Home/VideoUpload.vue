<template>
  <div id="VideoUpload">
    ...
    <div class="from-item">
      <span class="laber">视频：</span>
      <Upload
        ref="upload"
        multiple
        type="drag"
        :format="['avi','rmvb','rm','asf','divx','mpg','mpeg','mpe','wmv','mp4','mkv','vob']"
        :max-size="2048000"
  ...
  methods: {
    ...
    handleFormatError(file) {
      this.$Notice.warning({
        ...
        desc:
          "File format of " + file.name + " is incorrect, please select video."
      });
    },
    handleMaxSize(file) {
      this.$Notice.warning({
    ...
    handleBeforeUpload() {
      ...
      if (!check) {
        this.$Notice.warning({
          title: "Up to one video can be uploaded."
        });
      }
      return check;
    }
